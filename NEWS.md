...
- **The affiliation is no longer read off the wrong part of the address (#431)**: `AU_UN` is built by splitting each address in `C1` into its comma-separated parts and keeping the one that carries an institution tag (`UNIV`, `INST`, `HOSP`, `CTR`, ...). The parts holding digits are discarded first, since a street number, a PO box or a postal code is never an institution — but the positions of the tagged parts referred to the full address and were then used to index the shortened one, so every address whose institution was preceded by a numbered part returned a later part instead. `3400 Spruce St, Univ Penn, Philadelphia, PA USA` gave *PHILADELPHIA*, `PO Box 100, Univ Oslo, Oslo, Norway` gave *OSLO*, `Bldg 10 Room 2C146, Natl Cancer Inst, Bethesda, MD USA` gave *BETHESDA*: the affiliation silently became the city, sometimes the country, sometimes nothing at all. Street numbers and postal codes are the rule in Scopus, PubMed and Lens addresses and are common in the WoS reprint address as well. The parts are now indexed on the address they came from. In the package's reference collections this changes the affiliation of between 0.2% and 2.4% of the documents, in every case from a city, a country or a missing value to the institution actually stated in the address.
- **`AU1_UN` no longer reports `NOTREPORTED` as an affiliation (#431)**: when no institution can be recognised in a part of the address, the selection marks it `NOTREPORTED` (nothing matched) or `NOTDECLARED` (the only match was a number). These are internal markers. `AU_UN` stripped them before returning, `AU1_UN` — the corresponding author's affiliation — did not, so they reached the analyses and the reports as though they were the name of an institution: in the package's Scopus reference collection **414 of the 487** documents carried one. Both fields are now cleaned by the same routine, which also repairs the case that leaked through `AU_UN` too — a value made of markers only survived as a literal `NOTREPORTED`, because the bare markers were turned into `NA` before the compound ones were removed. `AU1_UN` now also picks the institution with the same routine as `AU_UN` instead of giving up on the first tagged part when that one holds a digit.
