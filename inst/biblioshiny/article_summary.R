...
get_article_summary_prompt <- function(summary_type) {
  ...
  prompt <- switch(summary_type,
                   ...
                   "thematic_bibliography" = {
                     "Analyze the bibliography/reference list of the attached scientific article. Identify the main research themes present in the cited works and group the references accordingly. For each theme, provide a brief description followed by a list of the corresponding citations from the article's bibliography."
                   },
                   ...
                   "list_tables_figures" = {
                     "Scan the entire document and extract a complete list of all tables and figures. For each item, provide its number and its full title or caption exactly as it appears in the text (e.g., 'Table 1: Participant Demographics' or 'Figure 1: Conceptual Model')."
                   },
                   # Default case for invalid input
                   {
                     stop(paste("Invalid 'summary_type'. Please use one of:",
                                "short_abstract, narrative_abstract, imrad_summary,",
                                "thematic_bibliography, research_questions, background_literature,",
                                "methods_summary, implications, list_tables_figures"))
                   }
  )
  prompt <- paste0(prompt," Do not include any additional commentary or information beyond what is requested. ",
