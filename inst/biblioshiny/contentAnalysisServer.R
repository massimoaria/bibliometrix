...
Avg sentence length: %.1f words",
  ...
  observeEvent(input$generate_summary, {
    ...
    if (!nzchar(api_key)) {
      showNotification(
        HTML("<strong>API Key Required</strong><br/>
    ...
    tryCatch({
      ...
      if (is.character(result) && length(result) == 1) {
        if (grepl("^❌", result) || grepl("Error", result, ignore.case = TRUE)) {
          showNotification(
            HTML(paste0("<strong>AI Request Failed</strong><br/>", 
                        gsub("❌ ", "", result))),
            type = "error",
            duration = 10
          )
      ...
      showNotification(
        HTML(paste0("<strong>Summary Generated!</strong><br/>",
                    "Type: ", summary_label)),
        type = "message",
        duration = 5
      )
      ...
      showNotification(
        HTML(paste0("<strong>Error Generating Summary</strong><br/>",
                    e$message)),
        type = "error",
        duration = 8
      )
  ...
  output$summary_content_display <- renderUI({
    ...
    summary_text <- if (is.character(values$biblioai_summary) && length(values$biblioai_summary) > 1) {
    ...
    summary_html <- gsub("\\*\\*([^*]+)\\*\\*", "<strong>\\1</strong>", summary_html)
    
    # Convert *text* to italic
    summary_html <- gsub("\\*([^*]+)\\*", "<em>\\1</em>", summary_html)
    
    # Convert ### headers to h4
    summary_html <- gsub("^### (.+)$", "<h4 style='color: #2E86AB; margin-top: 20px; margin-bottom: 10px;'>\\1</h4>", 
                         summary_html, perl = TRUE)
    
    # Convert ## headers to h3
    summary_html <- gsub("^## (.+)$", "<h3 style='color: #2E86AB; margin-top: 25px; margin-bottom: 10px;'>\\1</h3>", 
                         summary_html, perl = TRUE)
    
    # Convert # headers to h2
    summary_html <- gsub("^# (.+)$", "<h2 style='color: #2E86AB; margin-top: 30px; margin-bottom: 15px;'>\\1</h2>", 
                         summary_html, perl = TRUE)
    ...
    summary_html <- gsub("(<li>.*?</li>)\n(<li>)", "\\1\\2", summary_html, perl = TRUE)
    summary_html <- gsub("(<li>.*?</li>)", "<ul style='margin-left: 20px;'>\\1</ul>", summary_html, perl = TRUE)
    ...
    content = function(file) {
      if (!is.null(values$biblioai_summary)) {
        ...
        header <- paste0(
          ...
          "BiblioAI Summary\n",
          "====================================\n\n",
          "Summary Type: ", values$biblioai_summary_type, "\n",
          "Generated: ", format(values$biblioai_summary_timestamp, "%Y-%m-%d %H:%M:%S"), "\n",
          "Document: ", input$pdf_file$name, "\n\n",
          "====================================\n\n"
        )
